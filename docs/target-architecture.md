...
**Game-rule transitions live in the engine:** When the game ends, `tick()` sets `outcome` and `sim_state = Paused`. When a crisis appears, `tick()` sets `active_crisis` and `sim_state = Event { was_running }`. On disease detection or region collapse, `tick()` sets `sim_state = Paused`. When a crisis is resolved, `crisis::resolve_crisis()` restores `sim_state` from `Event { was_running }` back to Running or Paused. The engine owns the full lifecycle — entry and exit. The UI layer does not touch `sim_state`.

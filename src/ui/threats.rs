...
fn render_disease_detail(lines: &mut Vec<Line>, state: &GameState, disease_idx: usize) {
    ...
    lines.push(Line::from(vec![
        ...
        Span::raw("  "),
        Span::styled(format!("{:>8}", "Recov"), hdr),
        Span::raw("  "),
    ...
    for region in &state.regions {
        if let Some(inf) = region.infection_by_disease(disease_idx) {
            total_infected += inf.infected;
            total_recovered += inf.recovered;
            total_dead += inf.dead;
            ...
            lines.push(Line::from(vec![
                ...
                Span::raw("  "),
                Span::styled(
                    format!("{:>8}", format_number(inf.recovered)),
                    Style::default().fg(Color::Green),
                ),
                Span::raw("  "),
                Span::styled(
    ...
    lines.push(Line::from(vec![
        ...
        Span::raw("  "),
        Span::styled(
            format!("{:>8}", format_number(total_recovered)),
            Style::default().fg(Color::Green),
        ),
        Span::raw("  "),
        Span::styled(

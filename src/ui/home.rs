...
fn build_splash_content(state: &AppState) -> Vec<(String, Style)> {
    ...
    segments.push(("\n".to_string(), dim));
    segments.push(("  The board has granted you provisional authority\n".to_string(), white));
    segments.push(("  over research, containment, and budget.\n".to_string(), white));
    segments.push(("  How long that lasts depends on results.\n".to_string(), dim));
    segments.push(("\n".to_string(), dim));
...
fn render_dashboard(f: &mut Frame, area: Rect, state: &AppState) {
    ...
    for (i, disease) in state.diseases.iter().enumerate() {
        ...
        let total_inf: f64 = state.regions.iter().enumerate().map(|(ri, r)| {
            ...
            r.screened_infected_for_disease(i, &state.diseases, shows_exposed)
        }).sum();

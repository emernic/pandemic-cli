...
pub(super) fn build_crisis_event(state: &GameState, kind: CrisisKind) -> CrisisEvent {
    ...
    let event = match &kind {
        ...
        CrisisKind::ContractOffer { .. } => {
            ...
            let article = if contract_name.starts_with(|c: char| "AEIOUaeiou".contains(c)) { "an" } else { "a" };
            ...
            CrisisEvent {
                ...
                description: format!(
                    ...
                    member_name, article, contract_name, condition_desc, income_day, other_count,
                ),
                options: vec![
                    CrisisOption {

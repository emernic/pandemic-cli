...
/// `persistence::load_or_create()`. Do not call this from random startup,
/// test, or feature code. If a new caller seems necessary, stop and rethink
/// the bootstrap boundary first instead of adding a third path.
...
///
/// This is the constructor to use when you need a real fresh game outside the
/// save/load path.
...
/// `initialize_game()` itself). Do not open-code `WorldState::new_default()`
/// + `initialize_game()` + `AppState` assembly at another call-site. If this
/// function is missing something, extend this function instead.
...
pub fn execute_command(state: &mut WorldState, cmd: &GameCommand) -> CommandResult {
    ...
    let mut result = match cmd {
        ...
        GameCommand::ToggleQueueTech { tech } => {
            if let Some(pos) = state.queued_techs.iter().position(|t| t == tech) {
                state.queued_techs.remove(pos);
                CommandResult { message: Some(format!("Removed {} from queue", tech.name())), success: true, events: Vec::new() }
            } else {
                state.queued_techs.push(*tech);
                CommandResult { message: Some(format!("Queued {} — will auto-start when ready", tech.name())), success: true, events: Vec::new() }
            }
        }

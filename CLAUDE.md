...
- **`jq` is not installed** — do not pipe `gh` output through `jq`. Use `--json` flags with Python instead. Example: `gh issue list --json number,title,labels | python3 -c "import sys,json; [print(i['number'], i['title']) for i in json.load(sys.stdin)]"`
- **After squash-merging a PR, do not push follow-up commits to the same branch** — the squash rewrites history and GitHub will report the branch as non-mergeable. Either open a fresh branch for follow-up work, or `git rebase origin/master && git push --force-with-lease` before creating the follow-up PR.

...
> **CRISIS RULE: Every crisis is a decision. Read it, make a real choice, and report on it.** When a crisis fires, the game stops and shows the full event text and all options. Read everything before pressing anything. Navigate to your choice with `--key up`/`--key down`, then confirm with `--key enter`. In your report, record: what the crisis asked, what options were available, what you chose, and whether it felt like a meaningful decision or noise. That last assessment is the most valuable part — a crisis that feels like a no-brainer is feedback just as much as one that feels genuinely hard.
>
> Never put `--do enter` in the same invocation as `--do d<N>`. You cannot know what crisis is coming before it fires. Advance time in one invocation, stop, read the full output — if a crisis appeared, respond to it in the next invocation before advancing again.
>

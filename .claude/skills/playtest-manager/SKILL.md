...
> **CRISIS RULE: Read every crisis event before dismissing it.** When a crisis fires, the game stops and shows you the full event text and options. Read it. Navigate the options with up/down. Make a deliberate choice. If a crisis feels tedious, unimportant, or like the options don't matter — say so explicitly in your report. That is valuable feedback. What is NOT acceptable: pre-loading `--do enter` steps to blow past crises you haven't read. Do NOT chain `--do d<N> --do enter` in a single invocation unless you have already read this exact crisis type in this session and knowingly accept the default. Advance in short increments (2-3 days at a time) so you can read each crisis as it fires.
>

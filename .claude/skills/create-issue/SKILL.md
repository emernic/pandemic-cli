...
0. **Root causes before symptoms.** Before filing ANY issue, ask: "Is this a root cause, or a symptom of something bigger?" If the game lasts 5 minutes instead of 60, don't file issues about mid-game UX — the mid-game doesn't exist yet. File ONE issue about the broken game duration. Ten symptom issues are worth less than one root-cause issue. When in doubt, ask: "Would this problem still exist if [the bigger problem] were fixed?" If the answer is "maybe not," don't file it.

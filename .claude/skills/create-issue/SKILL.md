...
0. **Root causes before symptoms.** Before filing ANY issue, ask: "Is this a root cause, or a symptom of something bigger?" If the game lasts 5 minutes instead of 60, don't file ten issues about mid-game balance — file ONE issue about the broken game duration and note that your balance feedback is suspect because of it. But **still file genuinely independent issues** — a real bug (defeat triggers at the wrong time), a real UX problem (no feedback when toggling policies), a real clarity issue (vaccination vs treatment is confusing) — these are valid regardless of game duration. The key distinction: is your feedback *about the balance/pacing* (downstream of the root cause) or *about something that's broken independent of balance* (valid on its own)?

1. **⚠️ Do NOT file issues that add penalties or modifiers to existing features to address balance.** This is the most common failure mode in our issue backlog. If a feature feels "too strong" or "always the right choice," the reflex is to file an issue adding a cost, drain, penalty, or modifier to that feature. **Do not do this.** It makes the game worse: (a) every feature becomes a fuzzy mess of modifiers that blur together, (b) the player can't clearly reason about what anything does, (c) the codebase becomes impenetrable. Examples of issues you must NOT file:
   - "Quarantine should drain board approval over time" — tacking a penalty onto quarantine
   - "Auto-deploy should require personnel" — penalizing a QoL toggle to nerf a strategy
   - "Conscript Researchers should reduce research speed" — adding a downside to offset an upside
   - "Contracts should have compliance overhead costs" — making contracts fuzzier instead of redesigning them
   - "Model overdispersion / waning immunity / cross-immunity" — simulation realism with no player-facing decision

   **What to do instead:** Strategic depth comes from a small number of systems with clear rules that interact to create emergent complexity — like chess, where 6 piece types with simple movement rules create infinite depth. If a feature is always the right choice, the answer is never "add a penalty." The answer is: what system interaction would make this decision contextual? What visible, understandable mechanic would make the right answer depend on the situation? If you can't think of one, do not file an issue. **We have a solid backlog. When in doubt, do not file.**

2. **Never claim more than you actually know.** If the user said "the numbers look wrong," write "the numbers look wrong" — do NOT inflate that into detailed claims about which specific computation is wrong, what the correct values should be, or why the bug exists. Reading source code does NOT mean you know what happens at runtime. Skimming a function does NOT mean you understand the full data flow. **State observations at the level of confidence you actually have.** When in doubt, be vaguer, not more specific. A vague-but-true issue is infinitely better than a specific-but-wrong one.
3. **Describe what you actually experienced.** What were you doing? What happened? What was confusing, broken, or missing? Write from your real experience — do not fabricate or embellish.
4. **Quote what you literally saw.** Include the exact text string, error message, or UI label — not your interpretation of it. "The briefing reappears" is vague; "the `── BRIEFING ──` header reappears on the home screen after dismissing a crisis" gives the implementer something to grep for. For snapshot-mode observations, include the command you ran.
5. **Do NOT prematurely plan the solution.** The issue describes a *problem* or *need*, not an implementation plan. Resist the urge to design the fix. The person working on this issue will figure out the right approach themselves.
6. **If you have a solution idea**, you may include it, but it MUST go in a clearly marked section: `## Possible Solution (do not use directly)`. This section should be treated as a rough sketch, not a spec. The implementer should feel free to ignore it entirely.
...
**When the user requested an issue**, say "user" in the body — not "project owner," "maintainer," or other synonyms. Other agents search for the word "user" to identify these.
